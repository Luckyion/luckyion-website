...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		str += '<input type="radio" name="id" id="'+node.name+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +'" class="cx"/>';
	}
	...
	if (node.url) {
		...
		str += '<a onclick="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node">';
